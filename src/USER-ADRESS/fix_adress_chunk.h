...
FixStyle(adress/chunk,FixAdressChunk)

#else

#ifndef LMP_FIX_ADRESS_CHUNK_H
#define LMP_FIX_ADRESS_CHUNK_H

#include "fix.h"

namespace LAMMPS_NS {

class FixAdressChunk : public Fix {
 public:
  FixAdressChunk(class LAMMPS *, int, char **);
  ~FixAdressChunk();
  int setmask();
  void init();
  void setup(int);
